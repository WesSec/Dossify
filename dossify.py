...
def takedown(playlistid):
    ...
    r = session.post(url="https://contentreporting-api.spotify.com/v2/content-policy-report-form/get-code", json={"email": email})
    ...
    session.post(url="https://contentreporting-api.spotify.com/v2/content-policy-report-form/validate-code", json={"email":email,"resourceUri":resourceURI,"code":code})
    ...
    takedownjson = {"verificationDetails":{"resourceUri":resourceURI,"code":code,"googleRecaptchaToken": user_answer['solution']['gRecaptchaResponse']},"report":{"reporterEmail":email,"reporterCountryCode":"us","productUri":f"spotify:playlist:{playlistid}","contentType":"CONTENT_TYPE_PLAYLIST_TITLE","reason":"REASON_HATEFUL_OR_ABUSIVE","reporterUserAgent":"","experiments":[],"queryParams":{}}}
    # Send takedown request

    r = session.post(url="https://contentreporting-api.spotify.com/v2/content-policy-report-form/submit-external-report", json = takedownjson)
    # If http request succeeds, show text
    if r.status_code == 200 or r.status_code == 201:
        print("[!] Playlist taken down successfully, costs of takedown: $ " + str(user_answer['cost']))
